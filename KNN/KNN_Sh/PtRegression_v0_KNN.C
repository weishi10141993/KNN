...
///
///     root -l PtRegression_v0_KNN.C\(\"LD,MLP\"\)
///
...
/// - Package   : TMVA
/// - Root Macro: PtRegression_v0_KNN
///
/// \macro_output
/// \macro_code
...
const int MAX_EVT =   10000000;
const int REPORT_EVT = 1000000;
...
void PtRegression_v0_KNN( TString myMethodList = "" )
{
   ...
   //
   //     mylinux~> root -l PtRegression_v0_KNN.C\(\"myMethod1,myMethod2,myMethod3\"\)
   //
   ...
   std::cout << std::endl;
   std::cout << "==> Start PtRegression_v0_KNN" << std::endl;
   ...
   TString outfileName( "PtRegression_v0_KNN.root" );
   ...
   fnames.push_back("/afs/cern.ch/work/w/wshi/public/TMVA_2017/TMVA/EMTFPtAssign2017/mix/EMTF_MC_NTuple_SingleMu_noRPC_combined.root");
  ...
  // in_vars.push_back( MVA_var( "dPhi_24",   "#phi(4) - #phi(2)", "deg", 'F', -88 ) );
   ...
   //in_vars.push_back( MVA_var( "dTh_12",  "#theta(2) - #theta(1)", "deg", 'F', -88 ) ); // 0x1000 0000
   //in_vars.push_back( MVA_var( "dTh_23",  "#theta(3) - #theta(2)", "deg", 'F', -88 ) ); // 0x2000 0000
   //in_vars.push_back( MVA_var( "dTh_34",  "#theta(4) - #theta(3)", "deg", 'F', -88 ) ); // 0x3000 0000
   //in_vars.push_back( MVA_var( "dTh_13",  "#theta(3) - #theta(1)", "deg", 'F', -88 ) ); // 0x1000 0000
   //in_vars.push_back( MVA_var( "dTh_14",  "#theta(4) - #theta(1)", "deg", 'F', -88 ) ); // 0x2000 0000
   //in_vars.push_back( MVA_var( "dTh_24",  "#theta(4) - #theta(2)", "deg", 'F', -88 ) ); // 0x3000 0000
  ...
  // in_vars.push_back( MVA_var( "outSt",     "Outlier station",   "int", 'I', -88 ) );
   ...
   for (UInt_t iEvt = 0; iEvt < regChain->GetEntries(); iEvt++) {
     ...
     Double_t th1 = (track_br->GetLeaf("hit_theta"))->GetValue(4*iTrk + 0);
     Double_t th2 = (track_br->GetLeaf("hit_theta"))->GetValue(4*iTrk + 1);
     Double_t th3 = (track_br->GetLeaf("hit_theta"))->GetValue(4*iTrk + 2);
     Double_t th4 = (track_br->GetLeaf("hit_theta"))->GetValue(4*iTrk + 3);
     ...
     Double_t dTh12 = acos( cos( (th2 - th1)*(PI/180.) ) );
     dTh12 *= ( sin( (th2 - th1)*(PI/180.) ) / max( BIT, abs( sin( (th2 - th1)*(PI/180.) ) ) ) );
     dTh12 *= (180./PI);
     Double_t dTh13 = acos( cos( (th3 - th1)*(PI/180.) ) );
     dTh13 *= ( sin( (th3 - th1)*(PI/180.) ) / max( BIT, abs( sin( (th3 - th1)*(PI/180.) ) ) ) );
     dTh13 *= (180./PI);
     Double_t dTh14 = acos( cos( (th4 - th1)*(PI/180.) ) );
     dTh14 *= ( sin( (th4 - th1)*(PI/180.) ) / max( BIT, abs( sin( (th4 - th1)*(PI/180.) ) ) ) );
     dTh14 *= (180./PI);
     Double_t dTh23 = acos( cos( (th3 - th2)*(PI/180.) ) );
     dTh23 *= ( sin( (th3 - th2)*(PI/180.) ) / max( BIT, abs( sin( (th3 - th2)*(PI/180.) ) ) ) );
     dTh23 *= (180./PI);
     Double_t dTh24 = acos( cos( (th4 - th2)*(PI/180.) ) );
     dTh24 *= ( sin( (th4 - th2)*(PI/180.) ) / max( BIT, abs( sin( (th4 - th2)*(PI/180.) ) ) ) );
     dTh24 *= (180./PI);
     Double_t dTh34 = acos( cos( (th4 - th3)*(PI/180.) ) );
     dTh34 *= ( sin( (th4 - th3)*(PI/180.) ) / max( BIT, abs( sin( (th4 - th3)*(PI/180.) ) ) ) );
     dTh34 *= (180./PI);
   ...
   if (Use["KNN"])
      factory->BookMethod( dataloader,  TMVA::Types::kKNN, "KNN",
                           "!H:!V:nkNN=4:ScaleFrac=0.8:SigmaFact=1.0:Kernel=Gaus:UseKernel=T:UseWeight=T:!Trim" );
...
int main( int argc, char** argv )
{
   ...
   PtRegression_v0_KNN(methodList);
   return 0;
}
