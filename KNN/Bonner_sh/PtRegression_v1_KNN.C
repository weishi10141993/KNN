...
void PtRegression_v1_KNN ( TString myMethodList = "" ) {
   ...
   std::cout << std::endl;
   std::cout << "==> Start PtRegression_v1_KNN" << std::endl;
   ...
   TString out_dir = "/afs/cern.ch/work/w/wshi/public/TMVA_2017/TMVA/EMTFPtAssign2017";
   // out_dir = ".";
   TString out_file_name;
   out_file_name.Form( "%s/PtRegression_v1_KNN.root", out_dir.Data() );
   ...
   // Optimized mode 14 - WSHI 24.02.17, weight indicated by _invPt, input dPhi12, 23, theta, FR1, Bend_1, target 1/pT;
   factories.push_back( std::make_tuple( nullF, nullL, "f_0x0000110d_0x2_invPt", var_names, var_vals, 0x0000110d, 0x2) );
   ...
   in_vars.push_back( MVA_var( "dTh_34",  "#theta(4) - #theta(3)", "int", 'I', -88 ) ); // 0x4000 0000
   ...
   for (int iCh = 0; iCh < in_chains.size(); iCh++) {
	 ...
	 } else {//this marks the ZeroBias Data, need to use charge/eta/phi/pt to identify ZeroBias in test sample for rate plot
	   mu_pt     = 999.;
	   mu_eta    = -99.;
	   mu_phi    = -99.;
	   mu_charge = -99;
   ...
   for (UInt_t iFact = 0; iFact < factories.size(); iFact++) {
     ...
     // PDE - RS method
     if (Use["PDERS"])
        factX->BookMethod( loadX,  TMVA::Types::kPDERS, "PDERS",
     ...
     if (Use["PDEFoam"])
        factX->BookMethod( loadX,  TMVA::Types::kPDEFoam, "PDEFoam",
                               "!H:!V:MultiTargetRegression=F:TargetSelection=Mpv:TailCut=0.001:VolFrac=0.0666:nActiveCells=500:nSampl=2000:nBin=5:Compress=T:Kernel=None:Nmin=10:VarTransform=None" );
       
       // K-Nearest Neighbour classifier (KNN)
     if (Use["KNN"])
       factX->BookMethod( loadX,  TMVA::Types::kKNN, "KNN",
                               "!H:!V:nkNN=4:ScaleFrac=1:SigmaFact=1.0:Kernel=Gaus:UseKernel=T:UseWeight=T:!Trim" );
...
int main( int argc, char** argv )
{
   ...
   PtRegression_v1_KNN(methodList);
   return 0;
}
